...
from u_io import write_declare_action_state
...
class DeepBot(BasePokerPlayer):
    def __init__(self, id_=1, network='6max_full', full_dict = None, validation_mode=None, validation_dir='./simul_data/simul_0/gen_0', validation_id=None):
        ...
        if full_dict == None:
            ...
            self.full_dict = reduce_full_dict(full_dict=self.full_dict, network=self.network)
        else:
    ...
    def prep_input(self, hole_card, round_state, valid_actions, network = 'hu_first'):
        ...
        if network =='hu_first' or  network=='hu_second':
            ...
            inputs[7] = call_amount/(call_amount+tot_pot) # inputs[7] = call_price/(call_price+tot_pot)
