...
print('## Starting ##')
## prepare first gen deepbots and decks
print('\n LSTM BOT')
bot_ids=[1,2]
...
for gen_id in range(0):
    ...
    for bot_id in bot_ids:
        ...
        mutant_flat = mutate_bots(orig_bots_flat=[deepbot_flat], nb_new_bots=1,
    ...
    for bot_id in bot_ids:
        ...
        with open(gen_dir+'/bots/'+str(bot_id)+'/bot_'+str(bot_id)+'_flat.pkl', 'rb') as f:
            ...
            deepbot = DeepBot(id_=bot_id, gen_dir = gen_dir, full_dict = deepbot_dict, network=my_network, validation_mode = 'crossover_variance', validation_id=bot_id-1)
        while True:
            config = setup_config(max_round=max_round, initial_stack=3000, small_blind_amount=50)
            config.register_player(name="p1", algorithm=deepbot)
...
for gen_id in range(nb_dif_bots):
    for i,validation_id in enumerate(validation_ids):
        ...
        with open(gen_dir+'/bots/2/bot_2_flat.pkl', 'rb') as f:
            deepbot_flat_2 = pickle.load(f)
        if validation_id==2:
            cross_flat = crossover_bots_odd([deepbot_flat,deepbot_flat_2], ref_full_dict = ref_full_dict, nb_new_bots = 1)[0]
        if validation_id==3:
            cross_flat = crossover_bots_avg([deepbot_flat,deepbot_flat_2], ref_full_dict = ref_full_dict, nb_new_bots = 1)[0]
        if validation_id==4:
            cross_flat = crossover_bots_nosplit([deepbot_flat,deepbot_flat_2], ref_full_dict = ref_full_dict, nb_new_bots = 1)[0]
        if validation_id==5:
            cross_flat = crossover_bots_reg_200([deepbot_flat,deepbot_flat_2], ref_full_dict = ref_full_dict, nb_new_bots = 1)[0]
        if validation_id==6:
            cross_flat = crossover_bots_reg_250([deepbot_flat,deepbot_flat_2], ref_full_dict = ref_full_dict, nb_new_bots = 1)[0]


        cross_dict = get_full_dict(all_params = cross_flat, ref_full_dict = ref_full_dict)
        cross_bot = DeepBot(id_=bot_id, gen_dir = gen_dir, full_dict = cross_dict, network=my_network, validation_mode = 'crossover_variance', validation_id=validation_id)
        while True:
            config = setup_config(max_round=max_round, initial_stack=3000, small_blind_amount=50)
