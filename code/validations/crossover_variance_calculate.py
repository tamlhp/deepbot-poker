...
for gen_id in range(5,nb_dif_bots):
    ...
    with open(gen_dir+'/outputs_0.pkl', 'rb') as fr:
        try:
            while True:
                outputs_deepbot_1.append(pickle.load(fr))
        except EOFError:
            pass
    with open(gen_dir+'/outputs_1.pkl', 'rb') as fr:
        try:
            while True:
                outputs_deepbot_2.append(pickle.load(fr))
        except EOFError:
            pass
...
outputs_deepbot_1=np.array(outputs_deepbot_1)
outputs_deepbot_2=np.array(outputs_deepbot_2)
outputs_dif=[0,]*4
outputs_outside=[0,]*4
for i, validation_id in enumerate(validation_ids):
    outputs_crossover[i]=np.array(outputs_crossover[i])

    outputs_deepbot_dif = np.absolute(outputs_deepbot_1-outputs_deepbot_2)
    outputs_deepbot_mean = np.average([outputs_deepbot_1,outputs_deepbot_2], axis=0)
    outputs_dif[i]=np.absolute(outputs_crossover[i]-outputs_deepbot_mean)
    ...
    outputs_max = np.max([outputs_deepbot_1,outputs_deepbot_2],axis=0)
    outputs_min = np.min([outputs_deepbot_1,outputs_deepbot_2],axis=0)
    dif_min = np.min([np.absolute(outputs_crossover[i]-outputs_deepbot_1),np.absolute(outputs_crossover[i]-outputs_deepbot_2)],axis=0)
    ...
    print('avg dif between parent outputs: '+str(np.average(outputs_deepbot_dif)))
    print('')

parent_dist = np.average(outputs_deepbot_dif)/2
...
parent_dist = np.average(outputs_deepbot_dif)
plt.plot([0,5],[parent_dist,parent_dist], linewidth='1',color='lightgreen')
