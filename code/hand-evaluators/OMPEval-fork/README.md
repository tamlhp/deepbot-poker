...
This implementation is for using OMPEval's equity calculator in python. The contribution is one library libhandequity.so which can be imported in python through ctypes. The motivation is that no python library is nearly as fast. Here is a simple example of how to use it in python:
```python
...
hole_card = "9sTs"  # 9 of spades and ten of spades
...
```
The library's directory may have to be updated.

To rebuild both libraries (libompeval.a and libhandequity.so) run the script:
